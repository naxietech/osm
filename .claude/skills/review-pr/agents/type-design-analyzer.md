...
- Types should make illegal states unrepresentable (in TypeScript: discriminated unions, branded types, `readonly`, non-optional fields over optional-plus-runtime-check)
- Shared shapes belong in `@oses/types` (the single source of truth, zero deps); on the api, prefer `z.infer` from a Zod schema over a hand-declared parallel interface
- Constructor / boundary validation is crucial for maintaining invariants
